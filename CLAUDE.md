...
## Teardown — the domain is NOT destroyed automatically
...
**It does NOT remove the identity domain or the self-registration profile** —
Terraform cannot delete identity domains, and the profile is created out-of-band
by `setup_domain.sh`. **Clean these up manually in the console when done:**

1. Identity & Security → Domains → `notes-app` → User management → Self
   registration → delete the `spa-signup` profile (if it lingers).
2. Domain page → **Actions → Deactivate**, then **Actions → Delete**.

(External User domains bill per monthly active user, so delete the domain when
you're finished with the demo.)
